...
function LoanForm() {
    ...
    return <div>
        ...
        <form onSubmit={handleForm} id="form">
            <div>
                <p>ФИО:</p>
                <input id='FIO' placeholder='ФИО' required pattern="^([А-Я][а-я]{2,}\s[А-Я][а-я]{2,}\s[А-Я][а-я]{2,}+)|([A-Z][a-z]{2,}\s[A-Z][a-z]{2,}\s[A-z][a-z]{2,})$"
                   onChange={e => setFio(e.target.value)}/>
            <p>Паспорт</p>
            <div id="passport">
                <input id='passS' type='number' placeholder='Серия' required min={1000} max={9999}
                       onChange={e => setPassportSeries(Number.parseInt(e.target.value))}/>
                <input id='passN' type='number' placeholder='Номер' required min={100000} max={999999}
                       onChange={e => setPassportNumber(Number.parseInt(e.target.value))}/>
                <input id='passO' placeholder='Кем выдан' required minLength={10} maxLength={40}
                       onChange={e => setPassportGiven(e.target.value)}/>
                <input id='passD' placeholder='Дата выдачи' type='date' required min='1900-01-01' max='2022-01-01'
                       onChange={e => setPassportGivenDate(new Date(e.target.value))}/>
                <input id='passP' placeholder='Прописка' required minLength={5} maxLength={40}
                       onChange={e => setPassportRegistration(e.target.value)}/>
            </div>
            <p>Возраст:</p>
            <input id='age' type='number' placeholder='Возраст' required min={18} max={100}
